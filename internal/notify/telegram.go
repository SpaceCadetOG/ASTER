...
func NewTelegramFromConfig(fileKV map[string]string) *Telegram {
...
func (t *Telegram) SendTrade(event TradeEvent, isEntry bool) {
	...
	t.Sendf("%s %s %s\nsetup=%s conf=%.2f reason=%s price=%.6f",
		kind, strings.ToUpper(strings.TrimSpace(event.Symbol)), strings.ToUpper(strings.TrimSpace(event.Side)),
		strings.TrimSpace(event.Setup), event.Confluence, strings.TrimSpace(event.Reason), event.Price)
}
...
func (t *Telegram) enqueue(chatID, msg string) {
	...
	out := outboundMessage{chatID: chatID, text: msg}
	select {
	case t.msgCh <- out:
	default:
		// Non-blocking by design: drop when queue is saturated.
	}
}
